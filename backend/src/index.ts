...
async function runPythonExtractPoints(filePath: string, maxPoints: number = 100000) {
  ...
  return await new Promise<{ points: number[][] }>((resolve, reject) => {
...
async function main() {
  ...
  app.post("/api/scans/upload", upload.single("file"), async (req, res) => {
    ...
    if (ext === '.las' && !validateLasFile(req.file.path)) {
      ...
      return res.status(400).json({ error: "Invalid LAS file. Please upload a valid LiDAR point cloud file." });
    }
  ...
  app.get("/api/scans/:id/file", async (req, res) => {
    const id = String(req.params.id);
  ...
  app.get("/api/scans/:id/points", async (req, res) => {
    const id = String(req.params.id);
    const scan = await ScanModel.findById(id).lean();
    if (!scan?.filePath) return res.status(404).json({ error: "Scan not found" });
      ...
      const r = await fetch(
        `https://generativelanguage.googleapis.com/v1/models/gemini-2.5-flash:generateContent?key=${apiKey}`,
        {
          ...
          body: JSON.stringify({
            contents: [
              {
                parts: [{ text: `You are an expert construction progress assistant for a building project tracking system called Construction Tracker. The system tracks construction progress using point cloud scans and provides insights through comparisons, reports, and AI recommendations.
