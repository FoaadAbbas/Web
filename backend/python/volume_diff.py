...
def extract_points(path: str, max_points: int = 100000):
    ...
    try:
        if ext in (".las", ".laz"):
            ...
            points = np.vstack((las.x, las.y, las.z)).astype("float64")
        else:
...
def main():
    ...
    ap.add_argument("--max_extract_points", type=int, default=100_000)  # For extraction mode
    args = ap.parse_args()

    try:
        if args.extract:
            # Single file extraction mode
            points = extract_points(args.extract, args.max_extract_points)
            out = {"points": points}
